    ...
    <div className="py-20 md:py-32 bg-[#141414] overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 md:px-12 mb-12 md:mb-24 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 md:gap-8">
        <div className="max-w-2xl">
          ...
          >
            <span className="text-[#FFFFFF]/60 font-bold tracking-[0.4em] uppercase text-[12px] mb-3 md:mb-4 block">
              Strategic Offerings
            </span>
          </motion.div>
          <div className="overflow-hidden">
            <motion.h2
              ...
              className="text-4xl md:text-7xl font-bold leading-tight tracking-tighter text-[#FFFFFF]"
            >
              Strategic Systems Built for <br className="hidden md:block" />Scale, Control, and Longevity
            </motion.h2>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-8 items-start md:items-end">
          ...
          >
            Each engagement is a custom-built influence system designed to compound value over time. No templates. No short-term plays.
          </motion.p>
      ...
      <div className="relative group">
            ...
            >
              ...
              <div className="aspect-[16/10] relative overflow-hidden">
                ...
                <div className="absolute top-4 right-4 md:top-10 md:right-10 z-20">
                  <motion.div
                    ...
                    className="w-10 h-10 md:w-14 md:h-14 rounded-full bg-black/40 backdrop-blur-2xl border border-white/10 flex items-center justify-center text-[#FFFFFF] transition-colors hover:bg-white hover:text-black"
                  >
                ...
                <div className="absolute bottom-4 left-4 md:bottom-10 md:left-10 z-20">
                  <div className="bg-[#FFFFFF]/10 backdrop-blur-xl px-4 py-1.5 md:px-6 md:py-2 rounded-xl md:rounded-2xl border border-white/10">
                    <p className="text-[11px] text-gray-400 uppercase tracking-[0.3em] font-black">Outcome</p>
                    <p className="text-sm md:text-base font-bold text-[#FFFFFF]">Strategic Value</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 md:p-12 relative z-20">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 md:mb-12 gap-4 md:gap-0">
                  <div>
                    <h3 className="text-2xl md:text-4xl font-black mb-1 uppercase tracking-tighter text-[#FFFFFF] leading-none">{car.name}</h3>
                    <p className="text-[#FFFFFF]/60 font-black text-[11px] md:text-[12px] tracking-[0.4em] uppercase">{car.brand}</p>
                  </div>
                  <div className="w-full md:w-auto text-left md:text-right border-t border-white/5 md:border-none pt-4 md:pt-0">
                    <p className="text-xl md:text-2xl font-black text-[#FFFFFF] tracking-tighter">
                      Private Engagement
                    </p>
                  </div>
                </div>
                ...
                <div className="mb-8 md:mb-12">
                  <p className="text-white/60 text-sm md:text-base font-light leading-relaxed">
                    {index % 4 === 0 && "Reposition your brand from participant to category leader. We define your narrative, authority lane, and market perception."}
                    {index % 4 === 1 && "Turn attention into owned audiences, subscribers, and repeatable revenue systems across platforms you control."}
                    {index % 4 === 2 && "Web, apps, memberships, and ecosystems that eliminate platform dependency and centralize value."}
                    {index % 4 === 3 && "Protect and stabilize your brand with rapid-response reputation frameworks and controlled public narratives."}
                  </p>
                </div>

                <div className="grid grid-cols-3 gap-3 md:gap-6 mb-8 md:mb-12">
                  {[
                    { icon: <Zap size={18} />, label: car.id === '1' ? 'Market Positioning' : car.id === '2' ? 'Audience Ownership' : car.id === '3' ? 'Platform Ownership' : 'Risk Mitigation', val: car.power },
                    { icon: <Gauge size={18} />, label: car.id === '1' ? 'Authority Mapping' : car.id === '2' ? 'Subscriber Growth' : car.id === '3' ? 'Data Control' : 'Narrative Control', val: car.topSpeed },
                    { icon: <Flame size={18} />, label: car.id === '1' ? 'Brand Narrative' : car.id === '2' ? 'Revenue Architecture' : car.id === '3' ? 'Ecosystem' : 'Reputation', val: car.acceleration }
                  ].map((stat, i) => (
                    <motion.div
                      key={i}
                    ...
                    >
                      <div className="text-[#FFFFFF]/80 mb-2 md:mb-4 md:w-6 md:h-6">{stat.icon}</div>
                ...
                <motion.button
                  ...
                  className="w-full flex items-center justify-between px-6 py-5 md:px-12 md:py-7 bg-[#FFFFFF] text-black rounded-2xl md:rounded-[28px] transition-all duration-300 font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-[12px] md:text-[13px] overflow-hidden group/btn relative shadow-xl"
                >
                  <span className="relative z-10">Request Strategy Session</span>
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
          <motion.div
            ...
            className="text-[11px] font-black uppercase tracking-[0.4em] text-[#FFFFFF]/30"
          >
            Swipe to view more
          </motion.div>
        </div>
